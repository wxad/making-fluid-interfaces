...
- **重要：尽量使用 GPU 加速的 CSS 动画，而不是 JS API**。除非你在做一种交互动画。  
  _我的经验是 CSS `transition` 一般不需要优化，在低端机型上表现也非常好。而 JS 差距非常大，除非你在做基于 Spring 的交互动画，一般并不需要选择 `motion` 这样的库。_
...
- 不要使用或批判地看待类 `ease-in` 曲线，不要把交互动画和非交互（视觉）动画混为一谈。  
  _为什么我关闭一个弹窗，要先慢后快呢？一辆汽车开出我的视野，它是先慢后快的，但我关闭弹窗应该越快越好。区别是我不再是被动的观看者，而是主动的操作者。我的建议是只使用 `ease-out`。_

- 同时考虑曲线和时间，抛开一个谈另一个是没有意义的。重要的是保证动画可被打断或跳过。  
  _什么交互动画必须 `200ms` 以下的说法都没有意义，抛开曲线表现差异很大的。我经常用 `0.7s` 的 `transition`。_
...
- 如果要实现头部颜色过渡，则必须以 JS 的方式设置过渡，也就是在 1 秒内设置 60 次。  
  _因此如果有其他过渡需要和头部变色同步，那么都需要放弃 CSS `transition`，统一使用这个偷鸡方法。_
